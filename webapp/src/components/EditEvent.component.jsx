...
const EditEvent = ({ open, onClose, showSnackbar, event, fetchEvents }) => {
  // dayjs config
  ...
  useEffect(() => {
    if (open && event) {
      ...
      setStartDateTime(dayjs(new Date(event.startDateTime)));
      setEndDateTime(dayjs(new Date(event.endDateTime)));
      setSearchQuery('');
    }
  }, [open, event]);
  ...
  const handleUpdate = async () => {
    ...
    if (response.ok) {
      showSnackbar('success', event.isSuggested ? 'Event approved successfully!' : 'Event updated successfully!');
      onClose();
      fetchEvents();
    } else {
      setLoading(false);
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle>
        <Box className="dialog-title-text-container">
          <EditCalendarIcon />
          <Box>{event.isSuggested ? 'Approve event' : 'Update event'}</Box>
        </Box>
      </DialogTitle>
      ...
      <DialogContent sx={{ paddingBottom: 0 }}>
        <Box>
          <DialogContentText sx={{ paddingBottom: '16px' }}>
            {event.isSuggested
              ? 'Review and approve this suggested event. You can edit the details before approving it for everyone to see.'
              : 'Edit the details of this event. Changes will reflect immediately for all users.'}
          </DialogContentText>
      ...
      <DialogActions>
        ...
        <Button loading={loading} variant="contained" disableElevation onClick={handleUpdate}>
          {event.isSuggested ? 'Approve event' : 'Update event'}
        </Button>
      </DialogActions>
    </Dialog>
  );
